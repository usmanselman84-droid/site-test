...
export default async function Home() {
  ...
  return (
    <div className="home-page home-page--lift">
      <p className="home-build-version" aria-label={`Версия ${APP_VERSION}`}>
        версия {APP_VERSION}
      </p>
      {heroMediaKind === 'image' ? (
        <link
            ...
            <div>
              ...
              <p className="home-section-sub">
                {siteSettings?.publicEventsVisibility
                  ? 'Календарь открыт гостям. Запись на событие — после входа.'
                  : 'Гостям видна заглушка. Полный календарь — после входа (включается в настройках сайта).'}
              </p>
            </div>
