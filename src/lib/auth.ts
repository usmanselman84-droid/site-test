  ...
  providers: [
    CredentialsProvider({
      ...
      credentials: {
        ...
        bindToken: { label: "bindToken", type: "text" },
      },
      async authorize(credentials) {
        ...
        const bindFromPayload = raw.bindToken;
        delete raw.bindToken;
        ...
        const { peekSessionUserId, readSsoBindToken } = await import("./sso-link");
        const sessionUserId =
          readSsoBindToken(String(credentials?.bindToken || bindFromPayload || "")) ||
          (await peekSessionUserId());
        if (existingAcc?.user) {
