...
export async function GET() {
  ...
  const [state, flags, access] = await Promise.all([
    listLinkedProviders(session.user.id),
    oauthProviderFlags(),
    getAccessSettings(),
  ]);
  return NextResponse.json({
    ...
    available: {
      ...
      esia: Boolean(flags.esia) && Boolean(access.esiaLoginEnabled),
      telegramBot: flags.telegramBot || '',
      telegramBotId: flags.telegramBotId || '',
    },
  });
}
