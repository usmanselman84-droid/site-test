...
function TelegramMark() {
  return (
    <svg className="yp-sso__mark" width="24" height="24" viewBox="0 0 24 24" aria-hidden>
      ...
      <path
        fill="#fff"
        d="M17.6 7.2 6.9 11.3c-.73.28-.72.67-.13.84l2.74.85 1.06 3.26c.13.36.24.5.49.5.32 0 .46-.15.64-.32l1.54-1.5 3.2 2.36c.59.32 1.01.15 1.16-.54l2.1-9.9c.22-.86-.33-1.25-.88-1.02Z"
      />
    </svg>
  );
}
...
export default function SocialAuthButtons({
  ...
  const hasTg = Boolean(oauth.telegram && (oauth.telegramBot || oauth.telegramBotId));
  ...
  const startTelegram = () => {
    ...
    const botId = String(oauth.telegramBotId || '').replace(/\D/g, '');
    const returnTo = `${origin}/login/telegram?callbackUrl=${encodeURIComponent(callbackUrl)}`;
    if (botId) {
  ...
  useEffect(() => {
    if (!hasTg || oauth.telegramBotId || !oauth.telegramBot || !hostRef.current) return;
      ...
      <div className="yp-sso__grid yp-sso__grid--icons">
        ...
        {hasTg ? (
          oauth.telegramBotId ? (
            ...
            <div ref={hostRef} className="yp-sso__tg" />
          )
        ) : null}
        {hasEsia ? (
          <button
