...
export default function DashboardSettingsHub({
  ...
  const savePrivacy = async (
    nextProfile: typeof profileVisibility,
    nextOnline: typeof onlineVisibility
  ) => {
    setProfileVisibility(nextProfile);
    setOnlineVisibility(nextOnline);
    setProfileSaving(true);
    try {
      const res = await fetch('/api/user/profile', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ profileVisibility: nextProfile, onlineVisibility: nextOnline }),
      });
      const json = (await readJsonSafe(res)) || {};
            ...
            {(
              ...
              <label key={opt.id} className={`profile-choice${profileVisibility === opt.id ? ' is-on' : ''}`}>
                <input
                  ...
                  onChange={() => void savePrivacy(opt.id, onlineVisibility)}
                />
                <span>
            ...
            {(
              ...
              <label key={opt.id} className={`profile-choice${onlineVisibility === opt.id ? ' is-on-green' : ''}`}>
                <input
                  ...
                  onChange={() => void savePrivacy(profileVisibility, opt.id)}
                />
                <span>
                  <strong>{opt.title}</strong>
                  <span>{opt.desc}</span>
