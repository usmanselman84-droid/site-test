...
**Задачи:**
1. В `layout.tsx` / `HideOnPaths`: не прятать хром на кабинете (или отдельный `CabinetChrome` в React).
2. Убрать inject-костыль после фикса в исходниках + rebuild.
3. Сообщения/edit — по-прежнему immersive без marketing header.
